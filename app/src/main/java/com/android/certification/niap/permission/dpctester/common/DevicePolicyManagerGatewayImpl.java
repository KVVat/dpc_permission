...
public final class DevicePolicyManagerGatewayImpl implements DevicePolicyManagerGateway {
    ...
    //Append for tester tool
    @Override
    public void setContentProtectionPolicy(int policy) {
        Log.d(TAG, "setContentProtectionPolicy(): " + policy);
        if (Build.VERSION.SDK_INT >= 35) {
            mDevicePolicyManager.setContentProtectionPolicy(mAdminComponentName,policy);
        }
    }
    @Override
